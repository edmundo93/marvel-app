...
import { addFavCharacter, removeFavCharacter } from "@/presentation/contexts/characters-context/actions"
...
const CharacterResume = (props: IProps) => {
    ...
    useEffect(() => {
        if (favCharacters?.length) {
            const isFav = favCharacters.some(character => props.character?.id === character.id)
            setFav(isFav)
        } else {
            setFav(false)
        }
    ...
    return props.character && <InfoBar className={styles.characterResume}>
        <div className={styles.resumeContent}>
            ...
            <div className={styles.resumeInfo}>
                <div className={styles.characterTitle}>
                    ...
                    <IconButton width={24} height={21.68} icon={<FavIcon width={24} height={21.68} selected={fav} />} onClick={favIconClickHandler} />
                </div>
